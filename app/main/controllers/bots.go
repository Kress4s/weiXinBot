...
// @Success 200 {object} models.Bots
// @Failure 403
// @router / [get]
func (c *BotsController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
	...
	l, count, err := models.GetAllBots(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = common.RestResult{Code: -1, Message: err.Error()}
	} else {
...
func (c *BotsController) Put() {
	...
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseInt(idStr, 0, 64)
	var v = bridageModels.Bots{ID: id}
	if err = json.Unmarshal(c.Ctx.Input.RequestBody, &v); err != nil {
		return
	}
	err = models.UpdateBotByID(&v)
}

// Delete ...
// @router /:id [delete]
func (c *BotsController) Delete() {
	var err error
	defer func() {
		if err != nil {
			c.Data["json"] = common.RestResult{Code: -1, Message: err.Error()}
	...
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseInt(idStr, 0, 64)
	err = models.DeleteBotByID(id)
}

// DeleteList ...
// @router /deletelist [delete]
func (c *BotsController) DeleteList() {
	...
	if err := models.MultiDeleteBotsByIDs(idslice); err == nil {
		c.Data["json"] = common.RestResult{Code: 0, Message: "ok"}
	} else {
		c.Data["json"] = common.RestResult{Code: -1, Message: err.Error()}
	}
